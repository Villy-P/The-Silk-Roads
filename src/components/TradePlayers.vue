<template>
    ...
    <div v-if="store.state.user?.requestTradeWith">
        <div class="m-auto w-11/12 text-center">You requested a trade with {{ store.state.user.requestTradeWith }}</div>
    ...
    <div v-else v-for="u in getOtherPlayersAtLocation()" :key="u.username" class="w-10/12 border-black border-2 mx-auto my-5 h-10 flex items-center relative cursor-pointer" @click="requestTrade(u)">
        <div class="px-3" v-if="!u.requestTradeWith && !u.tradingWith">Request Trade with {{ u.username }}</div>
...
<script lang="ts">
    ...
    export default class TradePlayers extends Vue {
        ...
        goBack() {
            ...
            this.store.state.socket?.emit('cancelRequestTrade', this.store.state.user?.username, this.store.state.user?.requestTradeWith);
        ...
        requestTrade(user: User) {
            if (user.requestTradeWith)
                return;
            this.store.state.user!.requestTradeWith = user.username;
            this.store.state.socket?.emit('requestTrade', this.store.state.user?.username, this.store.state.user?.requestTradeWith);
            this.store.state.socket?.emit('updateUser', this.store.state.user);
        }
